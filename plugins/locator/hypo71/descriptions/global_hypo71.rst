...
been implemented into SeisComP through the plugin mechanism. A new plugin hypo71
...
uncertainties, RMS, pick residuals ...) to SeisComP.
...
After running Hypo71, the output is converted into a SeisComP (SeisComPML) origin
object including some error measures. The following table shows how
the Hypo71 error measures are mapped to the SeisComP data model:
...
In addition to the native Hypo71 output a SeisComP origin object is created and
...
Verify that everything is properly set up in the file `${SEISCOMP_ROOT}/share/hypo71/input`
